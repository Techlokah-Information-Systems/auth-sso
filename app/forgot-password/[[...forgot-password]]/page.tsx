...
export function ForgotPasswordForm() {
  ...
  const handleSendCode = async (e: React.SyntheticEvent) => {
    ...
    try {
      const rateLimitResult = await checkRateLimit("forgot-password-send");
  ...
  const handleReset = async (e: React.SyntheticEvent) => {
    ...
    try {
      const rateLimitResult = await checkRateLimit("forgot-password-reset");
      if (!rateLimitResult.success) {
        setError(rateLimitResult.error || "Too many requests.");
        setLoading(false);
        return;
      ...
      if (result.status === "complete") {
        if (clerk.client?.id && result.createdSessionId) {
          await linkUserWithClient(result.createdSessionId, clerk.client.id);
        }
        ...
        setError(
          `Password reset incomplete. Status: ${result.status}. Check console logs.`,
        );
      }
    } catch (err: any) {
  ...
  const handle2FA = async (e: React.SyntheticEvent) => {
    ...
    try {
      const rateLimitResult = await checkRateLimit("forgot-password-2fa");
      if (!rateLimitResult.success) {
        setError(rateLimitResult.error || "Too many requests.");
        setLoading(false);
        return;
        ...
        <div className="flex w-full md:w-1/2 flex-col p-8 md:p-14 lg:px-20 justify-center h-full overflow-y-auto">
          ...
          {needs2FA ? (
            <form onSubmit={handle2FA} className="flex flex-col gap-4">
              ...
              <Button
                type="submit"
                className="w-full bg-[#7a5af8] hover:bg-[#6b4ce6] text-white h-14 rounded-[6px] font-medium transition-colors mt-2 text-base"
